...
namespace Controllers
{
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfo> GetLogDataBetween(ZkemClient objZkeeper, int machineNumber,DateTime F, DateTime T,string IP)
        {
            string dwEnrollNumber1 = "";
            int dwVerifyMode = 0;
            int dwInOutMode = 0;
            ...
            while(objZkeeper.SSR_GetGeneralLogData(machineNumber, out dwEnrollNumber1, out dwVerifyMode, out dwInOutMode, out dwYear, out dwMonth, out dwDay, out dwHour, out dwMinute, out dwSecond, ref dwWorkCode))
            {
                ...
                objInfo.MacIP = IP;
                /*if (dir == 0)*/ objInfo.State = dwInOutMode;
                /*if (dir == 1) objInfo.State = 1;
                if (dir == 255) objInfo.State = 255;*/
                objInfo.DateTimeRecord = inputDate.ToString("yyyy-MM-dd HH:mm:ss");
                if(inputDate.Ticks>=F.Ticks && inputDate.Ticks<=T.AddDays(1).Ticks)
                    lstEnrollData.Add(objInfo);
            }
